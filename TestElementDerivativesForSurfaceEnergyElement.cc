...
const unsigned int numberOfQuadraturePointsForSurfaceEnergy							= 3;
...
typedef Elements::SurfaceGammaElement::LinearTwoNodeSurfaceEnergyElement<numberOfQuadraturePointsForSurfaceEnergy>																				SurfaceEnergyElement;
...
int main(int arc, char *argv[]) {
	...
	SurfaceEnergyElement element(elementNodesForSurface,surfaceEnergyElementPropertiesAC,&quadratureRuleForSurface);
	
	PrimitiveVariablesForSurfaceElement primitives;
	
	for(unsigned int i = 0; i < 2; i++){
		primitives[i] = 1e-3 * TotalVariableVector::Random();
		primitives[i](2) += 0.5*cmax;
	}
